...
| nginx_client_header_buffer_size                         | str  |           | Sets the nginx client_header_buffer_size. Left unset by default.                                                                                                                                                                                                        |
...
| nginx_geoip2_auto_reload                                | str  |           | How often nginx re-reads the GeoIP2 database, so a refreshed database is picked up without a reload.                                                                                                                                                                    |
| nginx_geoip2_database_directory                         | str  |           | Directory the geoip2 database is stored in.                                                                                                                                                                                                                             |
| nginx_geoip2_database_download                          | bool |           | Whether this role downloads and refreshes the geoip2 database itself. Disable it to provide the database by other means.                                                                                                                                                |
| nginx_geoip2_database_file                              | str  |           | Filename of the geoip2 database, matching what `nginx_geoip2_database_url` serves.                                                                                                                                                                                      |
| nginx_geoip2_database_path                              | str  |           | Full path of the geoip2 database, built from `nginx_geoip2_database_directory` and `nginx_geoip2_database_file`.                                                                                                                                                        |
| nginx_geoip2_database_url                               | str  |           | The download location of the gzipped geoip2 database, cached by the Exonet package cacher.                                                                                                                                                                              |
| nginx_geoip2_database_version                           | str  |           | The version of the `geolite2-city` package the database is downloaded from, resolved from `nginx_geoip2_database_version_major_map`.                                                                                                                                    |
| nginx_geoip2_database_version_major                     | str  |           | The `geolite2-city` release line the database version is resolved from. Renovate keeps the map entry for this line up to date.                                                                                                                                          |
| nginx_geoip2_module_url                                 | str  |           | The download location of the ngx_http_geoip2_module source.                                                                                                                                                                                                             |
| nginx_geoip2_module_version                             | str  |           | The version of ngx_http_geoip2_module to compile, resolved from `nginx_geoip2_module_version_major_map`.                                                                                                                                                                |
| nginx_geoip2_module_version_major                       | str  |           | The ngx_http_geoip2_module release line the version is resolved from. Renovate keeps the map entry for this line up to date.                                                                                                                                            |
...
| nginx_geoip_version                                     | int  |           | Which GeoIP module is compiled and used: `1` (default) for the legacy GeoIP (dat) module, `2` for the GeoIP2 (MaxMind mmdb) module.                                                                                                                                     |
...
| nginx_robots_txt_agents_list                            | list |           | List of `{name, disallow, allow, crawl_delay}` dicts written into the generated robots.txt. `disallow` and `allow` accept a string or list of paths. Defaults to a curated list of AI crawlers all set to `Disallow: /`.                                                |
| nginx_robots_txt_file_default                           | str  |           | `"{{ nginx_robots_txt_base_path }}/default.txt"`                                                                                                                                                                                                                        |
| nginx_robots_txt                                        | bool |           | Whether nginx intercepts and serves `/robots.txt`. Set to `false` to let the application handle it. Supports domain and user-level overrides.                                                                                                                           |
| nginx_robots_txt_file_playbook                          | str  |           | Nginx path to a custom static robots.txt that takes priority over the role-generated default.                                                                                                                                                                           |
| nginx_robots_txt_sitemap_urls                           | list |           | Global list of sitemap URLs included in the generated default robots.txt. Override at domain level with `domain.nginx_robots_txt_sitemap_urls`.                                                                                                                         |
...
Both GeoIP modules are supported; `nginx_geoip_version` selects which one is compiled into nginx and used by the generated configuration:

- `1` (default): the bundled `ngx_http_geoip_module`, reading the legacy `.dat` database from `nginx_geoip_database_path`.
- `2`: the `ngx_http_geoip2_module`, reading a MaxMind `.mmdb` database from `nginx_geoip2_database_path`.

Everything built on top of GeoIP — the blocking above and the [tiered rate limiting](#tiered-rate-limiting) — behaves identically on both, only the country code lookup differs (`$geoip_country_code` versus a `$geoip2_country_code` defined by a `geoip2` block). Switching versions changes the configure line, so nginx is recompiled on the next run.

The GeoIP2 module needs an mmdb database on the server; nginx does not start without it, so the role fails in preflight with an explanation when the file is missing. By default the role downloads the free GeoLite2 City database, no MaxMind account required:

```yaml
nginx_geoip: true
nginx_geoip_version: 2
```

The database comes from the `geolite2-city` npm package through the Exonet package cacher, so it is fetched from our own mirror instead of the public CDN. The version is pinned in `nginx_geoip2_database_version_major_map` in `vars/main.yml`, which Renovate bumps along with the other tracked dependencies. Pinning is also what makes the cached copy addressable: the role only downloads (tens of megabytes) when that version differs from the version recorded on the server. The city database is used because that is what the source publishes; only its country code is read.

The database is installed as `/usr/share/GeoIP/GeoLite2-City.mmdb` and put in place with a move, so nginx keeps serving from the old file until the new one is complete. nginx re-reads the file every `nginx_geoip2_auto_reload`, so a refreshed database applies without a reload.

Set `nginx_geoip2_database_download: false` to provide the database by other means. The download location, filename and target directory live in `vars/main.yml`, so pointing the role at a different mmdb is a change to the role rather than to a host or group.
...
- Enabling this automatically compiles the GeoIP module selected with `nginx_geoip_version` and emits its `geoip_country`/`geoip2` directive.
...
### Proxy buffer overrides

The role's default proxy buffer sizes (`nginx_default_proxy_settings`, see [defaults/main.yml](defaults/main.yml)) are applied to every proxied `location` block. Sites whose upstream returns larger responses or headers than that default can handle (for example large JSON payloads) can override individual buffer directives on the domain, without changing the role defaults or affecting other vhosts:

```yaml
domains:
  - name: bigresponse.example.nl
...
Only the directives listed under `proxy_settings` are overridden; any directive from `nginx_default_proxy_settings` that is not listed is still applied. `proxy_settings` also applies to `uris` and `domain.upstream` locations, and to the main `location /` proxied to Anubis on domains protected by it. It does not apply to the Anubis `/.within.website/` challenge endpoint, which only ever serves small assets.

To change a default for every domain instead of a single one, set `nginx_global_proxy_settings`:

```yaml
nginx_global_proxy_settings:
  - name: proxy_buffer_size
    value: "32k"
```

`nginx_global_proxy_settings` is overridden by a domain's own `proxy_settings` in turn, so the precedence is: `proxy_settings` (per-domain) > `nginx_global_proxy_settings` (role-wide override) > `nginx_default_proxy_settings` (role default).
